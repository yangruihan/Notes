...
class ProgressBar:
    ...
    def __init__(self, min_value=0, max_value=None, min_refresh_period=0.5, width=30, name=''):
        ...
        self._task_length = int(
            max_value - min_value) if (min_value is not None and max_value is not None) else None
    ...
    def _flush(self):
        ...
        if self._terminated:
            ...
            print('\r' + '##{}({:d} + {:d} -> {:d}) Task Finished.'
                  'Time Cost: {:3d} h {:3d} min {:6.4} s; Average: '
                  '{:3d} h {:3d} min {:6.4} s '.format(
                      self._bar_name, self._task_length,
                      self._min, self._max,
                      tmp_hour, tmp_min, tmp_sec,
                      tmp_avg_hour, tmp_avg_min, tmp_avg_sec) + ' ##\n', end='')
            self._ended = True
            return True
        ...
        print('\r' + '##{}['.format(self._bar_name)
              + '-' * passed + ' ' * (self._bar_width - passed)
              + '] : {} / {}'.format(self._counter, self._max)
              + '  # Time Cost: {:3d} h {:3d} min {:6.4} s; '
              'Average: {: 3d} h {: 3d} min {: 6.4} s '.format(
                  tmp_hour, tmp_min, tmp_sec,
                  tmp_avg_hour, tmp_avg_min, tmp_avg_sec)
              if self._counter != self._min
              else
              '##{}Progress bar initialized  ##'.format(
                  self._bar_name), end='')
    ...
    def set_min(self, min_val):
        if self._max is not None:
            if self._max <= min_val:
                print("Target min_val: {} is larger than current max_val: {}".format(
                    min_val, self._max))
                return
    ...
    def set_max(self, max_val):
        if self._min is not None:
            if self._min >= max_val:
                print("Target max_val: {} is smaller than current min_val: {}".format(
                    max_val, self._min))
                return
    ...
    def update(self, new_value=None):
        ...
        if new_value != self._min:
            self._counter = self._max if new_value >= self._max else int(
                new_value)
            return self._flush()
